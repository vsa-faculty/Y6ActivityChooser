...
var myListDocID = PropertiesService.getScriptProperties().getProperty('myListDocID'); //Doc ID of working Google spreadsheet 
const emailTemplateId = PropertiesService.getScriptProperties().getProperty('emailTemplateId'); //Doc ID of email template document
...
function studentPostData(rowindex, postedStudentId, currentChoice, currentExtraData, sindex){
  ...
  // Server-side role check: only student accounts can save choices.
  var isStudentUser = getRowsMatching(data, studentEmailCol, thisUser).length > 0;
  if (!isStudentUser) {
    lock.releaseLock();
    return {error: {status: true, class: "bg-danger", msg: "Only student accounts can save choices."}, update: {}, sindex: sindex};
  }

  //read result row given by rowindex
  var rowStudentId = data[rowindex][0];
  var rowStudentEmail = data[rowindex][studentEmailCol];
  if ((rowStudentId == postedStudentId) && (rowStudentEmail == thisUser)){ //check the expected student row and account match
